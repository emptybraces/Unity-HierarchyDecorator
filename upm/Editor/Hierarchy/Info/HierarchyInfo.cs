...
namespace HierarchyDecorator
{
	...
	{
		protected const int INDENT_SIZE = 10;
		...
		protected void CalculateFontSizeToFitWidth(GUIStyle style, float width, string text)
		{
			// フォントサイズを1から順に試して、テキストが収まる最大のフォントサイズを見つける
			for (int i = 9; i <= 11; i++) // 適切な最大フォントサイズを設定
			{
				style.fontSize = i;
				var size = style.CalcSize(new GUIContent(text));
				if (width < size.x)
				{
					style.fontSize = i - 1; // 一つ前のサイズが収まる最大のサイズ
					break;
				}
			}
		}
